...
**如何找到 Image Builder（镜像构建）：**

1. **方法一：从 Sealos 桌面查找**
   - 返回 Sealos 桌面（点击左上角的 Sealos  logo 或"应用管理"旁边的返回按钮）
   - 在桌面应用列表中查找 **"镜像构建"** 或 **"Image Builder"** 图标
   - 如果找不到，尝试点击桌面底部的网格图标（所有应用），查看完整应用列表

2. **方法二：在应用商店中查找**
   - 点击桌面上的 **"应用商店"** 图标
   - 搜索 "镜像构建" 或 "Image Builder"
   - 如果找到，点击安装或打开

3. **方法三：直接使用应用管理**
   - 如果找不到 Image Builder，您也可以：
     - 先准备好 Docker 镜像（使用其他方式构建）
     - 或者使用 Sealos 提供的公共镜像
     - 直接在"应用部署"界面使用镜像地址

**找到 Image Builder 后：**
1. 点击进入 **"镜像构建"** 或 **"Image Builder"**
2. 点击 **"新建构建"** 或 **"创建任务"** 按钮
