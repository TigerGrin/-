...
```json
{
  ...
  "data": null
**重要说明**:
- 休息、调休、培训等特殊状态会跳过排班规则验证
- 验证接口返回 `valid: true` 时，前端可以创建排班
- 验证接口返回 `valid: false` 时，前端应显示错误信息并阻止创建
- 前端应在创建特殊状态（`off_duty`、`training`）时跳过验证调用，直接创建排班
...
| annualLeave | number | 否 | 年休（天数，浮点型，支持负数表示欠休，范围-365到365），默认0 |
| storedLeave | number | 否 | 存休（天数，浮点型，支持负数表示欠休，范围-365到365），默认0 |
...
**重要说明**:
- 返回数据必须包含 `_id` 和 `id` 字段（MongoDB ObjectId），用于后续删除和更新操作
- 返回数据必须包含 `shiftName` 字段，用于前端显示
- 特殊状态（`off_duty`、`training`）会正常返回，与其他班次类型格式一致
...
| shiftType | string | 是 | 班次类型，详见数据字典。特殊状态：off_duty（休息/调休）、training（培训） |
| shiftName | string | 否 | 班次名称，系统可根据shiftType自动生成。特殊状态名称：休息、调休、院内培训 |
...
**重要说明**:
- **支持同一天多个排班**：同一护士在同一天可以安排多个不同的班次（例如：白班 + 夜班、责任组长 + 治疗护士等）
- 休息、调休、培训等特殊状态会跳过排班规则验证
- 返回数据必须包含 `id` 和 `_id` 字段（MongoDB ObjectId），用于后续删除和更新操作
- 返回数据必须包含 `shiftName` 字段，用于前端显示
- 特殊状态 `shiftType` 可以为 `off_duty`（休息/调休）或 `training`（培训）
- 特殊状态的 `shiftName` 可以为：`"休息"`、`"调休"`、`"院内培训"`
...
**重要说明**:
- **支持同一天多个排班**：同一护士在同一天可以安排多个不同的班次，更新排班时不再检查是否已有排班
- 休息、调休、培训等特殊状态会跳过排班规则验证
- 验证接口返回 `valid: true` 时，前端可以创建排班
- 验证接口返回 `valid: false` 时，前端应显示错误信息并阻止创建
- 前端应在创建特殊状态（`off_duty`、`training`）时跳过验证调用，直接创建排班
...
| nurseId | string | 是 | 护士工号 |
| date | string | 是 | 排班日期，格式: YYYY-MM-DD |
| shiftType | string | 是 | 班次类型。特殊状态（off_duty、training）会跳过验证 |
...
**重要说明**:
- **支持同一天多个排班**：同一护士在同一天可以安排多个不同的班次，批量更新时不再检查是否已有排班
- 休息、调休、培训等特殊状态会跳过排班规则验证
- 验证接口返回 `valid: true` 时，前端可以创建排班
- 验证接口返回 `valid: false` 时，前端应显示错误信息并阻止创建
- 前端应在创建特殊状态（`off_duty`、`training`）时跳过验证调用，直接创建排班
...
#### 标准班次类型（8种）：
- `day_team_leader`: 责任组长(白班) - 10小时，工作日各1人，周末/节假日1人，必须由责任组长担任
...
**重要说明**：
- 特殊状态（`off_duty`、`training`）在创建排班时会跳过验证规则
- `shiftName` 字段用于区分同一 `shiftType` 下的不同状态（如休息vs调休）
- 所有班次类型必须包含 `shiftName` 字段用于前端显示
...
- **验证规则**：特殊状态（`off_duty`、`training`）会跳过排班规则验证，可以直接创建
- **班次名称**：特殊状态的 `shiftName` 由前端传递，用于区分同一 `shiftType` 下的不同状态

### 10. 年休和存休字段
- **支持负数**：`annualLeave` 和 `storedLeave` 支持负数（表示欠休），范围：-365 到 365
- **精度**：支持浮点型，精度为 0.01（如：10.5、-5.25）
- **默认值**：两个字段默认值为 0
...
8. **排班特殊状态**
   ...
   - 调休状态：`shiftType: "off_duty"` + `shiftName: "调休"`
   - 培训状态：`shiftType: "training"` + `shiftName: "院内培训"`
   - 特殊状态会跳过排班规则验证，可以直接创建
   - 创建排班时必须返回完整的排班对象，包含 `id`、`_id`、`shiftName` 等字段

9. **年休和存休字段**
   - `annualLeave` 和 `storedLeave` 支持负数（表示欠休），范围：-365 到 365
   - 支持浮点型，精度为 0.01（如：10.5、-5.25）
   - 默认值为 0
